...
public class OrderDetailManager implements OrderDetailService {
    ...
    private GetOrderDetailByIdResponse orderDetailMapping(int orderId, int productId, OrderDetail orderDetail) {
        ...
        DataResult<Order> order = this.orderService.findById(orderId);
        DataResult<Product> product = this.productService.findById(productId);
        getOrderDetailByIdResponse.setContactName(order.getData().getCustomer().getContactName());
        getOrderDetailByIdResponse.setOrderDate(order.getData().getOrderDate());
        getOrderDetailByIdResponse.setProductName(product.getData().getProductName());
    ...
    private OrderDetailListResponse orderDetailMappingList(int orderId, int productId, OrderDetail orderDetail) {
        ...
        DataResult<Order> order = this.orderService.findById(orderId);
        DataResult<Product> product = this.productService.findById(productId);
        orderDetailListResponse.setContactName(order.getData().getCustomer().getContactName());
        orderDetailListResponse.setOrderDate(order.getData().getOrderDate());
        orderDetailListResponse.setProductName(product.getData().getProductName());
